...
parser.add_argument("--log_dir", type=str, default="./otherlogs/", help='path of log files')
...
class Net(nn.Module):
    def __init__(self, model_name, mode):
        ...
        # self.pcn_loss = SoftIoULoss()
        if model_name == 'PCFNet':
            if mode == 'train':
                self.model = PCFNet(mode='train')
            else:
                self.model = PCFNet(mode='test')
    ...
    def loss(self, preds, gt_masks):
        if isinstance(preds, list):
            ...
            for i in range(len(preds)):
                pred = preds[i]
                loss = self.cal_loss(pred, gt_masks[i])
            ...
            #     pred = preds[0][i]
            #     iou_loss = iou_loss + self.pcn_loss(pred, gt_masks)
            return loss_total
